...
import com.sourcepointmeta.metaapp.tv.ui.toProperty
...
internal class AddUpdatePropertyViewModelTv(
    ...
    private val validationManager: ValidationManagerTv,
    ...
    fun createOrUpdateProperty(propertyTv: PropertyTvDTO, fieldType: PropertyField, newField: String?) {
        viewModelScope.launch {
            validateProperty(propertyTv)
                .map {
                    if (fieldType == PropertyField.PROPERTY_NAME) {
                        dataSource.deleteByPropertyName(propertyTv.property.propertyName)
                    }
                    val np = propertyTv.toProperty(fieldType, newField)
    ...
    fun fetchPropertySync(propertyName: String): PropertyTvDTO {
        return dataSource.fetchPropertyByNameSync(propertyName).toPropertyTvDTO()
    }

    private fun validateProperty(property: PropertyTvDTO): Either<PropertyTvDTO> = validationManager.run {
