...
class PvDataApiModelExtKtTest {
    ...
    fun `GIVEN the PvData body parameters RETURN a request body input`() {
        val testMapMSS = JSONObject("v7/messagesObj.json".file2String())
        val messResp = JsonConverter.converter.decodeFromString<MessagesResp>(testMapMSS.toString())
        ...
        sut.toTreeMap().run {
            getMap("gdpr")!!.let { g ->
                ...
                g.getFieldValue<String>("euconsent")!!.assertEquals("CPf9mIAPf9mIAAGABCENCiCgAAAAAHAAAAYgAAAMZgAgMZADCgAQGMhwAIDGRIAEBjIA.YAAAAAAAAAAA")
